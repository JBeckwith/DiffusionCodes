...
class CVE():
    ...
    def periodogram(self, x, deltaT, N):
        ...
        # N is number of data points
        # ================OUTPUT============= 
        # periodogram (see Sec B1 of paper) """
        from scipy import fft
        # get discrete sine transform; multiplication by 2 and deltaT are to
        # make directly comparable formulae
        DST = np.multiply(deltaT, np.multiply(0.5, fft.dst(np.diff(x), type=1))) 
        Pk = np.divide(np.multiply(2, np.square(DST)), np.multiply(deltaT, np.add(N, 1.)))
        return Pk
    
    def chi_periodogram(self, Pk):
        """ chi_periodogram function
        # calculates likelihood behaviour is diffusive using test from
        # Vestergaard, C. L.; Blainey, P. C.; Flyvbjerg, H
        # Phys. Rev. E 2014, 89 (2), 022726. 
        # https://doi.org/10.1103/PhysRevE.89.022726.
        # to estimate if particle is undergoing diffusive motion or not
        ...
        # Pk is periodogram
        # ================OUTPUT============= 
        # periodogram (see Sec B1, following eq 11 of paper) """
        from StatsFunctions import Statistics_Functions
        SF_F = Statistics_Functions()
        epsilon_k = Pk/np.mean(Pk)
        pdf, eps_vals = np.histogram(epsilon_k, bins=np.linspace(0, np.max(epsilon_k), int(np.around(np.max(epsilon_k)*2))), density=True)
        xcenters = (eps_vals[:-1] + eps_vals[1:]) / 2
        gamma = SF_F.gamma_pdf(xcenters, 0.5, 2.)
        chi = np.sum(np.divide((np.square(pdf - gamma)), gamma))
        return chi